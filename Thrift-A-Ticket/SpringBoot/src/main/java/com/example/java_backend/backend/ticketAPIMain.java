...
// For SeatGeek API:
// Your app secret is fab7dbffd504ffa3c4b334e9fc0c24b0957ebec644fb21557f73f2b7e9c05c43 - copy now as it cannot be retrieved later.
@Service
public class ticketAPIMain {
	private static final HttpResponse<String> NULL = null;
	public UserTickets callAPI(String user_event_name, String user_state_initials) {
		Scanner kb = new Scanner(System.in); {
			...
			String get_req_url = "https://app.ticketmaster.com/discovery/v2/events.json?keyword=" + user_event_name + "&countryCode=US&stateCode="
								 + user_state_initials + "&apikey=kp12R01fgG1JtbhuJmKX9tzmho9pYjDy";
			
			// Sending HTTP GET request to get the top 20 chart rankings from the online movie database
			HttpResponse<String> response = NULL;
			  ...
			  // Parsing the response for relevant data
			    ...
			    //System.out.println("PUBLIC TICKET SALE START DATE AND TIME: " + sale_start_date + " " + sale_start_time);
			    ...
			    //System.out.println("PUBLIC TICKET SALE END DATE AND TIME: " + sale_end_date + " " + sale_end_time);
			    ...
			    // getting the event's current status (onsale, offsale, canceled, postponed, or rescheduled)
			    ...
			    //System.out.println("EVENT'S CURRENT STATUS: " + status_string);
					return new UserTickets(null, event_name_string, event_name_string, event_date_string, event_time_string, "null", null, min_price_string, url_string);
				}
			}
		}
